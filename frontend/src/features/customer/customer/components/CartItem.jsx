...
const CartItem = ({ item }) => {
  ...
  return (
    <div 
      className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 border rounded-lg hover:shadow-md transition-all duration-200 group cursor-pointer ${
        themeMode === 'dark' 
    ...
    >
      {/* Product Image */}
      <div className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 bg-[var(--div)] rounded-lg overflow-hidden flex items-center justify-center">
        {firstImage ? (
          <img 
            src={firstImage} 
            alt={item.name} 
          ...
          <div className="w-full h-full bg-[var(--div)] flex items-center justify-center">
            <svg className="w-6 h-6 sm:w-8 sm:h-8 text-[var(--text)] opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 lg:gap-6">
        
        {/* Left Section - Product Info */}
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-[var(--text)] text-base sm:text-lg mb-1 line-clamp-2 sm:truncate">
            {item.name}
          </h3>
          <p className="text-[var(--text)] opacity-70 text-xs sm:text-sm">
            ${Number(item.price || 0).toFixed(2)} each
          </p>
          
          {/* Vendor Name */}
          {item.vendor_name && (
            <p className="text-xs text-[var(--text)] opacity-50 mt-1 hidden sm:block">
              Vendor: {item.vendor_name}
            </p>
          )}
        </div>

        {/* Middle Section - Quantity Counter */}
        <div className="flex items-center justify-between sm:justify-center gap-4 sm:gap-2">
          {/* Total Price - Show on mobile only */}
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                ...
                className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-md hover:bg-[var(--hover)] transition-colors text-[var(--text)] text-sm sm:text-base"
              >
              ...
              <span className="min-w-6 sm:min-w-8 text-center font-medium text-[var(--text)] text-base sm:text-lg">
                {item.quantity}
              </span>
        ...
        <div 
          className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4 w-full sm:w-auto"
          onClick={(e) => e.stopPropagation()} // منع النقر على هذا القسم من التوجيه
        >
          {/* Total Price - Hide on mobile (shown above) */}
          ...
          <button
            ...
            className="w-8 h-8 sm:w-10 sm:h-10 bg-[var(--error)] text-white rounded-lg hover:opacity-90 transition-opacity flex items-center justify-center flex-shrink-0"
            title="Remove item"
          >
